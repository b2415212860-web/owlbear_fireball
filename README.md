...
在场景就绪、未施法时，点击悬浮 UI 下方的“选择残留素材”，在 Owlbear 素材选择窗口选中 Fire Sphere 或其他 Props 图片/动画。窗口可能需要确认选择。面板出现“残留：素材名”后即启用；无需先拖入地图。此次只实现素材选择，不提供场景棋子设为模板入口。

- 同一浏览器、同一扩展地址、同一房间记住选择；不是全房间统一配置，不自动同步到其他玩家、浏览器或设备。
...
预期：manifest 返回 200、JSON 中 version 为 `1.3.0`；fx.html 返回 200 和 HTML 类型，并允许 Owlbear 跨域读取/嵌入。如果公网仍显示 Halo 404，检查 CDN 回源是否到 Nginx 80 而不是绕过它直连 Halo 8090，以及 www 和裸域是否配置一致。
...
- `residue.ts`：用户选择素材、按浏览器和房间保存模板、权限检查与一次性创建共享 Props。
